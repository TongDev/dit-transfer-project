<?php
    ...
    if (isset($_POST['insert'])) {
        ...
        $result = $check_codename->check_codename_a($cacName);
        $num = mysqli_num_rows($result);
        if ($num > 0) {
            echo "<script>alert('รหัสหลักสูตรซ้ำ, โปลดลองใหม่อีกครั้ง !');</script>";
            echo "<script>window.history.back();</script>";
        } else {
            $sql_insert = $insertdata->insert_a($cacName,$caName,$baName,$caImprove,$caCredit,$scNum);
        }
    ...
    if (isset($_POST['edit'])) {
        ...
        $sql = $insertdata->insert_school($schoolID,$schoolName);
        
        if ($sql) {
            echo "<script>alert('เพิ่มสถาบันเสร็จสิ้น !');</script>";
            echo "<script>window.location.href='course_a_insert.php'</script>";
        } else {
            echo "<script>alert('ลบสถาบันล้มเหลว !');</script>";
            echo "<script>window.location.href='course_a_insert.php'</script>";
        }
    }
...
<head>
    ...
    <title>เพิ่มหลักสูตรประกาศนียบัตรชั้นสูง</title>
    <!-- CSS -->
...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
    ...
    <!-- Form insert avp -->
    <div class="container">
        <h1 class="mt-3">เพิ่มหลักสูตรประกาศนียบัตรชั้นสูง</h1>
        <hr>
        <div class="container">
        ...
        <form action="" method="post">
            <div class="mb-3">
                <label for="courseA_CodeName" class="form-label">ชื่อย่อหลักสูตร</label>
                <input type="text" name="courseA_CodeName" class="form-control" required>
            </div>
            <div class="mb-3">
                <label for="courseA_Name" class="form-label">ชื่อหลักสูตร</label>
                <input type="text" name="courseA_Name" class="form-control" required>
            </div>
            <div class="mb-3">
                <label for="branchA_Name" class="form-label">สาขา</label>
                <input type="text" name="branchA_Name" class="form-control" required>
            </div>
            <div class="mb-3">
                <label for="courseA_Improve" class="form-label">ปีที่ปรับปรุง</label>
                <input type="text" name="courseA_Improve" class="form-control" required>
            </div>
            <div class="mb-3">
                <label for="courseA_Credit" class="form-label">หน่วยกิต</label>
                <input type="text" name="courseA_Credit" class="form-control" required>
            </div>
            <div class="mb-3">
                <label for="schoolName" class="form-label">สถาบัน</label>
                <select class="form-select" name="schoolNum" aria-label="โปรดเลือกสถาบัน" required>
                    <option selected>โปรดเลือกสถาบัน</option>
                    <?php
                    include_once('functions.php');
                    $fetchselect = new DB_con();
                    $sql = $fetchselect->fetchselect_school();
                    while($row = mysqli_fetch_array($sql)){
                    ?>
                    <option value="<?php echo $row['schoolNum']; ?>"><?php echo $row["schoolNum"];?>. <?php echo $row["schoolName"];?></option>
                    <?php
                        }
                    ?>
                </select>
            </div>
            <button type="submit" name="insert" class="btn btn-success mb-2">INSERT</button>
        </form>
            <button class="btn btn-warning" data-toggle="modal" data-target="#insert_school" data-whatever="@mdo">เพิ่มสถาบัน</button>
            <hr>

        <div class="modal fade" id="insert_school" tabindex="-1" aria-labelledby="insert_schoolLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="insert_schoolLabel">เพิ่มสถาบัน</h5>
                    ...
                    <div class="modal-footer">
                        <button type="submit" name="edit" class="btn btn-success">INSERT</button>
                    </div>
                        </form>
                    </div>
                </div>
