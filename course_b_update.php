<?php
    ...
    if (isset($_POST['update'])) {
        ...
        $sql = $updatedata->update_b($cbID,$cbcName,$cbName,$bName,$cbImprove,$cbCredit);
        ...
        if ($sql) {
            ...
            echo "<script>window.location.href='course_b_update.php'</script>";
        }
    }
?>
...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
    ...
    <div class="container">
        ...
        <?php
            $cbID = $_GET['courseB_ID'];
            $update_bcl = new DB_con();
            $sql = $update_bcl->fetchonerecord_b($cbID);
            while($row = mysqli_fetch_array($sql)) {
        ?>
        <form action="" method="post">
            <div class="mb-3">
                <label for="courseB_CodeName" class="form-label">ชื่อย่อหลักสูตร</label>
                <input type="text" name="courseB_CodeName" class="form-control"
                value="<?php echo $row['courseB_CodeName']; ?>" required>
            </div>
            <div class="mb-3">
                <label for="courseB_Name" class="form-label">ชื่อหลักสูตร</label>
                <input type="text" name="courseB_Name" class="form-control"
                value="<?php echo $row['courseB_Name']; ?>" required>
            </div>
            <div class="mb-3">
                <label for="branchB_Name" class="form-label">ชื่อสาขา</label>
                <input type="text" name="branchB_Name" class="form-control" 
                value="<?php echo $row['branchB_Name']; ?>" required>
            </div>
            <div class="mb-3">
                <label for="courseB_Improve" class="form-label">ปีที่ปรับปรุง</label>
                <input type="text" name="courseB_Improve" class="form-control" 
                value="<?php echo $row['courseB_Improve']; ?>" required>
            </div>
            <div class="mb-3">
                <label for="courseB_Credit" class="form-label">หน่วยกิต</label>
                <input type="text" name="courseB_Credit" class="form-control" 
                value="<?php echo $row['courseB_Credit']; ?>" required>
            </div>
            <?php } ?>
