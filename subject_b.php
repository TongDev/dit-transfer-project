<?php
    include_once('functions.php');//subject_b_search.php
    if (isset($_POST['search'])) {

    $search = $_POST['search'];
    $search_data = new DB_con();
    $sql_search = $search_data->search_sb($search);
    $num = mysqli_num_rows($sql_search);
    
        if ($num > 0) {
            $_SESSION['num'] = $num;
            echo "<script>alert('ค้นหาเสร็จสิ้น !');</script>";
            echo "<script>window.location.href='#'</script>";
        } else {
            echo "<script>alert('ไม่พบผลการค้นหา !');</script>";
        ...
        <div class="row justify-content-between">
            ...
            <div class="col-6">
                <form action="" method="post" class="d-flex">
                    <input class="form-control mr-2 mt-3" type="search" name="search" placeholder="ค้นหารายวิชา . . ." aria-label="Search">
                    <button class="btn btn-outline-success mt-3"" type="submit" name="btn_search">ค้นหา</button>
                </form>
            </div>
        </div>
        <hr>
        ...
        <table id="subject_b_table" class="table table-bordered table-striped">
            ...
            <tbody>
                <?php
                    ...
                    $sql = $fetchdata->fetchdata_b_subject();
