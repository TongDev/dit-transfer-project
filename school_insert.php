<?php
    ...
    if (isset($_POST['insert'])) {
        ...
        $check_schoolname = new DB_con();
        $result = $check_schoolname->check_schoolname($schoolName);
        $num = mysqli_num_rows($result);
        if ($num > 0) {
            echo "<script>alert('ชื่อสถาบันซ้ำ, โปลดลองใหม่อีกครั้ง !');</script>";
            echo "<script>window.history.back();</script>";
        } else {
            $sql_insert = $insertdata->insert_school($schoolID,$schoolName);
        }
