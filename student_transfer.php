<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        for ($i=0; $i < $num; $i++) {
            ...
            $chk_sm = new DB_con();
            $sql_smchk = $chk_sm->chk_sm($s[$i]);
            $row_chk_sm = mysqli_fetch_array($sql_smchk);
            $sm_num = $row_chk_sm['sMatchNum'];
            $sm_cre = $row_chk_sm['subject_ACredit'];
            $num_sm = mysqli_num_rows($sql_smchk);
            
            if ($g[$i] == 0) {
                /*echo $s[$i];
                echo "  ";
                echo "ไม่ได้เลือกเกรด";
                echo "<br>";*/
            } elseif ($g[$i] < 2) {
                /*echo $s[$i];
                echo "  ";
                echo "เกรดไม่ผ่านเกณฑ์";
                echo "<br>";*/
            } elseif ($num_sm < 1) {
                /*echo $s[$i];
                echo "  ";
                echo "ไม่มีวิชาที่เทียบได้";
                echo "<br>";*/
            } else {
                /*echo $std_id;
                echo "  ";
                ...
                $sql_insert = $insertdata->insert_grade($std_id,$ca_id,$s[$i],$g[$i],$sm_num,$sm_cre);
            }

        }
        if ($sql_insert) {
            echo "<script>alert('เพิ่มเกรดนักศึกษาเสร็จสิ้น !');</script>";
            echo "<script>window.location.href='#'</script>";
        } else {
            echo "<script>alert('เพิ่มเกรดนักศึกษาล้มเหลว !');</script>";
            echo "<script>window.location.href='#'</script>";
...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
    ...
    <div class="container">
        ...
        <div class="container">
            <div class="row justify-content-between">
                ...
                <div class="col-2">
                    <a href="transfered.php?student_id=<?php echo $id; ?>" class="btn btn-success mb-3">ผลการเทียบโอน</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="container">
        ...
        <table id="s_match_b_table" class="table table-bordered table-striped">
                ...
                <tbody>
                    <?php
                        ...
                        while ($row0 = mysqli_fetch_array($sql)) {
                            $sj_aid = $row0['subject_AID'];
                            $chk_grade = new DB_con();
                            $sql_chk_grade = $chk_grade->chk_grade($sj_aid);
                            $num_grade = mysqli_num_rows($sql_chk_grade);
                            if($num_grade >= 1){

                            } else {
                    ?>
                    <tr>
                        <td class="text-center"><?php echo $row0['courseA_CodeName']; ?></td>
                        <td class="text-center"><?php echo $row0['subject_ACode']; ?></td>
                        <td><?php echo $row0['subject_AName']; ?></td>
                        <td class="text-center"><?php echo $row0['subject_ACredit']; ?></td>
                        <td class="text-center" style="width: 150px;">
                            <select name="grade[]" class="form-select">
                                <option selected value="0">เลือกเกรด</option>
                    ...
                    <input type="text" name="subject_AID[]" hidden="true" value="<?php echo $row0['subject_AID']; ?>">
                </tbody>
                            <?php } ?>
                        <?php } ?>
            </table>
            <hr>
            <div class="row justify-content-center">
                <div class="col-6 text-center"><button class="btn btn-success btn-block" type="submit" name="submit">Submit</button></div>
